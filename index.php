...
include_once("/var/www/mileage/php/utilities/Common.php");
include_once("/var/www/mileage/php/utilities/RouteParser.php");

include_once("/var/www/mileage/php/DataAccess/UserAuth.php");
include_once("/var/www/mileage/php/DataAccess/DataAccess.php");
...
switch ($routeParser->ResourcePath()) {
    ...
    default:
        ...
        if (array_key_exists(4, $uriparts)) {
            ...
            $fillup_id = explode("/", $_SERVER['REQUEST_URI'])[4];
            $maintenance_id = explode("/", $_SERVER['REQUEST_URI'])[4];
            $reminder_id = explode("/", $_SERVER['REQUEST_URI'])[4];
            $trip_id = explode("/", $_SERVER['REQUEST_URI'])[4];
        }
        if (array_key_exists(6, $uriparts))
            $trip_checkpoint_id = explode("/", $_SERVER['REQUEST_URI'])[6];
        break;
}
